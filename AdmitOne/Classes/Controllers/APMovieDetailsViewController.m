...
-(void)updateViewWithMovie:(APMovie*)movie{
    ...
    _runtime.stringValue = [NSString stringWithFormat:@"Runtime : %d min",movie.runtime];
    _criticsRatingText.stringValue = [NSString stringWithFormat:@"%d%% %@",movie.criticsScore,movie.criticsRating];
    _audienceRatingText.stringValue = [NSString stringWithFormat:@"%d%% %@",movie.audienceScore,movie.audienceRating];
...
-(IBAction)qualitySelect:(id)sender{
    [[NSUserDefaults standardUserDefaults]setBool:[sender selectedSegment] forKey:@"hdQuality"];
    [[NSUserDefaults standardUserDefaults]synchronize];
    _movie.bestTorrent = nil;
    [self _findDownloadForCurrentLanguage];
}
